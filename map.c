...
int mapper(int *, int);		//performs the mapping operation	-	breaks the lines into words

void main(int argc, char **argv)		//collects lines from the file
{
	int store_p[P_SIZE], fd_p[2];
	int i = 0, j = 0, word_count;
	char buf[2];				//buffer to read the file
	fd_p[0] = atoi(argv[1]);		//Get fds as arguments. If used with combiner the pipe fds are passed
	fd_p[1] = atoi(argv[2]);		//If used as a standalone prog then usage is ./map 0 1
	char *count = (char *)malloc(sizeof(mapped_s *)), *store = (char *)malloc(sizeof(char));	//buffers for storing and printing results
	int file_t = open("read.txt", O_RDONLY);							//opens a handle to the file read.txt
	buf[1] = NULL;
	do												//this section reads entire lines from file
	{
		...
		{
			*(store + j) = NULL;			
			store_p[i] = (int)store;	//if the no. of lines exceed the prog capacity increase the size of store_p in lib_map_reduce.h
			i++;
			j = 0;
	...
	for(j = 0; j < word_count; j++)									//print sorted elements on the pipe
	{
		mapped_i = (mapped_s *)store_buf[j];
		realloc(count, sizeof(mapped_i->word));
		write(fd_p[1], "(", strlen("("));
		write(fd_p[1], mapped_i->word, strlen(mapped_i->word));
		sprintf(count, ", %d)\n", mapped_i->count);
		write(fd_p[1], count, strlen(count));
	}
	printf("Exiting Mapper\n");
	close(fd_p[1]);
}

int mapper(int *store_p, int i)
{
	int j, k, l, m = 0, n;
	printf("Entered mapper\n");
	for(j = 0; j < i; j++)
	{
		l = 0;
