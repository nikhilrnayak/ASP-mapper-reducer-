...
void sort(int *f_store_buf, int start, int end)					//quick sort for arranging alphabetically - is case sensitive
{
	if(start < end)
	{
		...
		for(i = start; i < end + 1; i++)
		{
			...
			(strlen((char *)mapped_v3->word) < strlen((char *)mapped_v1->word)) ? (limit = strlen((char *)mapped_v3->word)) : (limit = strlen((char *)mapped_v1->word));
			for(j = 0; j < limit; j++)
			{
